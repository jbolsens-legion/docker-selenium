...
if [ "${START_XVFB:-${SE_START_XVFB}}" = true ]; then
  if [ "${START_VNC:-${SE_START_VNC}}" = true ]; then
    ...
    for i in $(seq 1 10); do
      ...
      if /usr/bin/fbsetbg -u Esetroot -c /usr/share/images/fluxbox/ubuntu-light.png; then
        break
      fi
    done
    ...
    if [[ $(ulimit -n) -gt $TOO_HIGH_ULIMIT || -n "${SE_VNC_ULIMIT}" ]]; then
